...
- **Admin role/group/permission CRUD MCP tools.** 42A adds read-only `permission_catalog_list`. 42C adds eight read-only management tools: `role_list`, `role_get`, `role_members`, `group_list`, `group_get`, `permission_holders`, `employee_get`, `audit_log_list`. Combined with the existing `employee_grant_permission`, `employee_revoke_permission`, `employee_create`, `employee_disable`, `employee_assign_role`, `employee_revoke_role`, etc., admins can manage users + run compliance queries end-to-end via natural-language bot commands without hand-writing SQL. CRUD tools for *defining new roles and groups* (`role_create`, `group_update`, `role_add_group`, …) are deferred to a future Slice 42D — today operators define roles via platform-core seed + ad-hoc SQL. Add 42D when SQL becomes painful.
