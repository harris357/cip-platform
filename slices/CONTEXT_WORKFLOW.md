...
| **61** — Remove intent-classifier subsystem. Production data on 2026-05-06 showed LLM-only path is fast (1–2s) and accurate; classifier-off measurements proved the sklearn layer wasn't earning its keep. Deletes `packages/intent-classifier/`, `classifier-lifecycle/` module, retrain workflow, trace-import cron, helm chart, CI build job, bot's classifier-client + classify node. Preserves data tables (training_data, bot_intent_examples, model_runs, bot_turn_feedback) for future few-shot mining. Rewrites `/turn-feedback` to write Langfuse scores instead of training_data rows. **Cancels 58H.** | [SLICE_61_REMOVE_INTENT_CLASSIFIER.md](./SLICE_61_REMOVE_INTENT_CLASSIFIER.md) | DRAFTED |
