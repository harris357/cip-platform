...
| **59** — Messaging coherency. Reconciles two divergent NATS domain enums (subject builder vs stream creator) into one; replaces per-tenant streams with global per-domain streams (`CIP_*_EVENTS`); rebinds the hr-service watcher to actual stream names; promotes 3 fire-and-forget business publishes to `js.publish`; adds explicit consumer config (`max_deliver`, `ack_wait`, `max_ack_pending`) + DLQ pattern (`CIP_DLQ_EVENTS`). Substrate fix — does NOT implement TODO handlers. | [SLICE_59_MESSAGING_COHERENCY.md](./SLICE_59_MESSAGING_COHERENCY.md) | DRAFTED |
| **60** — Evaluation harness MVP (read-only analytics). 5 SQL views over `documents` + `audit_events` + extraction evidence; 3 MCP tools (`eval_classifier_summary`, `eval_extraction_field_coverage`, `eval_recent_reclassifications`); new `documents.admin.eval` permission; CSV download via signed S3 URL. Scope decisions logged in `EVAL_HARNESS_INVESTIGATION.md`. Holdout-set management (60-B), replay (60-C), matcher correctness (60-D), continuous-reporting cron (60-E) deferred. | [SLICE_60_EVAL_HARNESS.md](./SLICE_60_EVAL_HARNESS.md) | DRAFTED |
| **61** — Remove intent-classifier subsystem AND grammar/extractor pre-router (aggressive). Production data on 2026-05-06 showed LLM-only path is fast (1–2s) and accurate; both pre-LLM optimization layers (sklearn classifier slice 56 + grammar regex pre-router slice 55) are no longer earning their keep. Deletes `packages/intent-classifier/`, `classifier-lifecycle/` module, retrain workflow, trace-import cron, helm chart, CI build job, bot's `classifier-client` + `classify` node + **`intent/grammar/`** + **`intent/extractors/`** (10 files of HR-domain regex + per-tool arg extractors that violate "no domain logic in bot" + cross-DB queries to `cip_hr.employees`) + `grammar-route` node. **Drops** `training_data`, `bot_intent_examples`, `model_runs`, `bot_turn_feedback`, `grammar_router_metrics` tables. Rewrites `/turn-feedback` to write Langfuse scores only. **Cancels 58H.** Tool ambiguity moves to per-tool 422-response contract (follow-up slice). | [SLICE_61_REMOVE_INTENT_CLASSIFIER.md](./SLICE_61_REMOVE_INTENT_CLASSIFIER.md) | DRAFTED |
